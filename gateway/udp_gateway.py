...
def run_gateway(host: str, port: int, api_url: str, allowed_nodes: set[str], timeout_seconds: float) -> None:
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind((host, port))
    LOGGER.info("listening for UDP telemetry on %s:%s", host, port)
    LOGGER.info("relaying valid packets to %s", api_url)
    while True:
        ...
        try:
            ...
            relay_packet(api_url, normalized, timeout_seconds)
            LOGGER.info("relayed %s seq=%s rssi=%s", normalized["node_id"], normalized["sequence"], normalized["rssi"])
        except Exception:
...
def main() -> None:
    ...
    run_gateway(args.host, args.port, args.api_url, allowed_nodes, args.timeout)
